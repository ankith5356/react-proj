...
const App = () => {
  ...
  const submitHandler = (e) => {
    ...
    const copyTask = [...task]; //Making a copy of task  state

    copyTask.push({ title, details }); //Pushing Details to copyTask

    setTask(copyTask); // React ko nayi state de rahe hain → re-render trigger hota hai
    // console.log(task)  // Prints previous state
  ...
  return (
    <div className="h-screen lg:flex bg-black text-white ">
      <form onSubmit={(e) => {
        ...
        className="flex flex-col gap-5 lg:w-1/2 items-start p-10"
      >
        <h1 className="text-3xl font-bold">Add Notes :</h1>

        {/* First Add Note Section */}
        <input
          type="text"
          placeholder="Enter Notes Heading"
          className="px-5 py-2 w-full border font-medium outline-none rounded"
          value={title}
          onChange={(e) => {
        ...
        <textarea
          ...
          className="px-5 py-2 w-full font-medium h-25 items-start flex-row border outline-none rounded"
          value={details}
          onChange={(e) => {
        ...
        {/* Button Section */}
        <button className="bg-white w-full active:bg-amber-300 font-medium text-black px-5 py-2  rounded">
          Add Note
        </button>
      </form>
