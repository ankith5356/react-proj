import React, { useState } from "react";
import { X } from 'lucide-react';

const App = () => {
  const [title, setTitle] = useState("");
  const [details, setDetails] = useState("");

  const [task, setTask] = useState([]);

  const submitHandler = (e) => {
    e.preventDefault();

    const copyTask = [...task]; //Making a copy of task  state

    copyTask.push({ title, details }); //Pushing Details to copyTask

    setTask(copyTask); // React ko nayi state de rahe hain → re-render trigger hota hai
    // console.log(task)  // Prints previous state

    setTitle("");
    setDetails("");
  };

  return (
    <div className="h-screen lg:flex bg-black text-white ">
      <form onSubmit={(e) => {
          submitHandler(e);
        }}
        className="flex flex-col gap-5 lg:w-1/2 items-start p-10"
      >
        <h1 className="text-3xl font-bold">Add Notes :</h1>

        {/* First Add Note Section */}
        <input
          type="text"
          placeholder="Enter Notes Heading"
          className="px-5 py-2 w-full border font-medium outline-none rounded"
          value={title}
          onChange={(e) => {
            setTitle(e.target.value);
          }}
        />

        {/* Second Details Section */}
        <textarea
          type="text"
          placeholder="Write Details"
          className="px-5 py-2 w-full font-medium h-25 items-start flex-row border outline-none rounded"
          value={details}
          onChange={(e) => {
            setDetails(e.target.value);
          }}
        />

        {/* Button Section */}
        <button className="bg-white w-full active:bg-amber-300 font-medium text-black px-5 py-2  rounded">
          Add Note
        </button>
      </form>

      {/* Recent Notes Section */}
      <div className="lg:w-1/2 lg:border-l-2  bg-gray-900 p-10">
        <h1 className="text-3xl font-bold">Recent Notes : </h1>
        <div className="flex gap-5 flex-wrap items-start mt-5 h-full overflow-auto">
          {task.map((elem, idx) => {
            return (
              <div key={idx}
                className="flex justify-between flex-col items-start relative h-52 w-40  rounded-2xl
                 text-black pt-4 pb-5 px-3 bg-cover bg-[url('data:image/png;base64,')]" >
                <div>
                  <h3 className="leading-tight  text-xl font-bold">{elem.title}</h3>
                  <p className="mt-2 font-medium text-gray-400 leading-tight">{elem.details}</p>
                </div>
                 <button className="w-full cursor-pointer active:scale-95 bg-red-300 text-white 
                 rounded py-1 font-bold text-xs">Delete</button>
              </div>
            );
          })}
        </div>
      </div>
    </div>
  );
};

export default App;
// https://youtu.be/3LRZRSIh_KE?t=23367
// https://youtu.be/3LRZRSIh_KE?t=25329
// https://youtu.be/3LRZRSIh_KE?t=25849
